# A cobled together version of https://github.com/kaushalmodi/hello_musl/blob/master/config.nims with stuff commented out that have not been tested
# Only enabled thing is pcre, which is for regex
# run `nim musl -d:pcre <entry to application/ source file>`
# ensure CROSS_PREFIX var point to the your toolchain prefix. e.g.: 'host-tools/gcc/riscv64-linux-musl-x86_64/bin/riscv64-unknown-linux-musl-', the gcc part is removed from the end
...
putEnv("CC", getEnv("CROSS_PREFIX") & "gcc")
putEnv("CXX", getEnv("CROSS_PREFIX") & "g++")
putEnv("CCLD", getEnv("CROSS_PREFIX") & "ld")
putEnv("CXXLD", getEnv("CROSS_PREFIX") & "ld")
...
let
  ...
  pcreConfigureCmd = ["./configure", "--prefix=" & pcreInstallDir, "--enable-pcre16", "--enable-pcre32", "--disable-shared", "--host=x86_64"]
  ...
  # libreSslConfigureCmd = ["./configure", "--disable-shared", "--prefix=" & libreSslInstallDir, "--host=x86_64"]
...
#       putEnv("CC", getEnv("CC") & " -static -idirafter /usr/include/ -idirafter /usr/include/x86_64-linux-gnu/")
...
switch("passC", getEnv("CFLAGS") & " -static")
switch("passL", getEnv("CFLAGS") & " -static")
